...
| E2 | 正弦柔性双域 | in_progress（q0.99 v2聚合、跨频身份门、6条强迫边界定向二分与双层runner冻结均通过；E2-F2 q0.99预检、右删失上界扩展及72条双层细化均通过，q0.95/q0.90待执行） | E1；E2-F0；W1慢域、直接功率快域与发电机侧高频域均已有数据入口 | 按冻结双层runner契约执行三功率×三SOC扩展 |
...
- E2-F2-P0配置与per-ray括区runner审计通过：配置`configs/e2_f2_cross_operating_slow_v1.json`覆盖9个功率×SOC运行点、三频、两控制器和四相位，共216条射线；q0.99预检波次为72条。配置SHA-256为`98B3C08719A647559C3979B5BE867706017CFE9C65CFCAA5A8911D8FB3013053`，runner SHA-256为`949CBCC8F6A5D062467F2D37C7C1E2B12893A84A19C0BD36F24A1B2C0562D43D`，核心代码身份保持`C931...`；审计见`E2_F2_CROSS_OPERATING_AUDIT.md`。初始q0.99预检目录`runs/e2_f2_cross_operating_q099_precheck_v1/`完成72/72，但9条P=0.8 MPC射线在`0.3 p.u.`右删失；原始聚合保留不改写。
- E2-F2 q0.99右删失上界定向扩展已完成：扩展配置`configs/e2_f2_q099_mpc_upper_extension_v1.json`固定目标射线`[1,3,5,9,11,13,17,19,21]`，仅复用F2 q0.99预检中`≤0.3 p.u.`案例，新增`0.35 p.u.`并在首个强迫失效括区处停止；9/9报告、9/9新增案例、求解失败0、恢复阶段新增物理违规0、强迫阶段非星形0。9条射线均形成`[0.3,0.35] p.u.`强迫及联合括区；目标扩展门通过。72条合并复核聚合`runs/e2_f2_q099_mpc_upper_extension_v1/e2_f2_q0.99_precheck_extended_aggregate.json`的`precheck_gate_pass=true`，仅保留原有联合非星形诊断射线`[25,33,41]`。完整结果见`E2_F2_Q099_UPPER_EXTENSION_RESULTS.md`；下一步为q0.99逐射线细化二分。
- E2-F2 q0.99双层细化已完成：细化配置`configs/e2_f2_q099_refinement_v1.json`从合并预检`runs/e2_f2_q099_mpc_upper_extension_v1/e2_f2_q0.99_precheck_extended_aggregate.json`读取72条逐射线证据，分别对`forcing_stage_safe`与`joint_valid_for_boundary`二分，复用已有预检端点且不复用F1案例。运行目录为`runs/e2_f2_cross_operating_q099_refine_v1/`，72/72报告、252个新增中点案例，缺失0、重复0、求解失败0、恢复阶段新增物理违规0、强迫阶段非星形0；强迫括区宽度`0.0052—0.0085 p.u.`（中位数`0.0052875`），联合括区宽度`0.0052—0.0085 p.u.`（中位数`0.005475`），`refinement_gate_pass=true`。联合层非星形诊断仍为射线`[25,33,41]`，不改变强迫层门判定。完整结果见`E2_F2_Q099_REFINEMENT_RESULTS.md`和`runs/e2_f2_cross_operating_q099_refine_v1/e2_f2_q0.99_refine_aggregate.json`；q0.95为下一波次。
